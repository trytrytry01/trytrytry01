...
export class BuyerSignupComponent implements OnInit {
  ...
  onSubmit(value: any) {
    if (this.validInput(value)) {
      this.userService.buyerSignUp(value).subscribe(
        data => {
  ...
  validInput(value: any): boolean {
    ...
    if (!value.email) {
      this.alerts.push({type : 'danger', message: 'email is required!'});
      result =  false;
    }

    if (value.email.indexOf('@') == -1) {
      this.alerts.push({type : 'danger', message: 'email is invaild!'});
      result =  false;
    }
